...
@app.route("/")
def index():
    file_path = "logs/taxy_server.log"
    content = "<H1>TAXY Server is running</H1><br><b>Log file:</b><br>"
    content += (
        "Frame width: "
        + str(_FRAME_WIDTH)
        + ", Frame height: "
...
def drawOnFrame(usedFrame):
    ...
    if _camera_url is None:
        usedFrame = drawTextOnFrame(usedFrame, "TAXY Server Configuration not recieved.", row=2)
